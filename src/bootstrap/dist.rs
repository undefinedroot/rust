...
impl Step for Src {
    ...
    fn run(self, builder: &Builder<'_>) -> PathBuf {
        ...
        // and fix them...
        let dst_src = image.join("lib/rustlib/src/rust/src");
        ...
        //
        // `build_helper` is in `src` and the rest are in `library`, so we have these two lists.
        let std_src_dirs_src = ["build_helper"];
        let std_src_dirs_libs = [
            "alloc",
            "core",
            "panic_abort",
            "panic_unwind",
        ...
        copy_src_dirs(builder, &builder.src.join("src"), &std_src_dirs_src, &[], &dst_src);
        copy_src_dirs(builder, &builder.src.join("library"), &std_src_dirs_libs, &[], &dst_src);
